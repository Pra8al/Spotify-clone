...
import {AuthorV0, SaveSong, TitleVO} from '../service/model/song.model';
...
export class AddSongComponent implements OnDestroy {
  ...
  constructor() {
    effect(() => {
      ...
      if (this.songService.addSig().status === "OK") {
        ...
        this.toastService.show("Error occurred when created song, please try again", "DANGER");
      }
    });
  }
  ...
  create(): void {
    ...
    const authorVO: AuthorV0 = {value: this.createForm.value.author};
